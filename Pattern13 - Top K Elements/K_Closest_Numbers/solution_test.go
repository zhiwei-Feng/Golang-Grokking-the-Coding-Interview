...
func Test_findClosestElementsByTwoPointer(t *testing.T) {
	type args struct {
		arr []int
		k   int
		x   int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := findClosestElementsByTwoPointer(tt.args.arr, tt.args.k, tt.args.x); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("findClosestElementsByTwoPointer() = %v, want %v", got, tt.want)
			}
		})
	}
}
